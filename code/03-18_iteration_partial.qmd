...
nematode %>% 
  ...
  ggplot()+
  geom_density(aes(x = value, fill = name), show.legend = F)+
...
meancv <- function(x){
  ...
  print(data.frame(mean = mean, cv = cv))
}
...
```{r}
nematode_meancv <- data.frame(variable = rep(NA, times = 26),
                          cv = rep(NA, times = 26),
                          mean = rep(NA, times = 26))
...
nematode_mods_sel_plots$splots_better[3]
```
